...
|---|---|---|
| `telegram_bot_token` | `TELEGRAM_BOT_TOKEN` | Token from BotFather. The agent stays online without it, but Telegram send/receive capabilities are disabled. |
...
| `telegram_routing_target` | `TELEGRAM_ROUTING_TARGET` | Agent name to forward incoming messages to. Defaults to `task-planner-agent`. |
...
| `TELEGRAM_ROUTING_TARGET` | `task-planner-agent` | Target agent for inbound messages |
