...
class OrchestratorClient:
    def __init__(self, base_url: str = "http://localhost:8000") -> None:
        ...
        self._agent_id_cache: dict[str, str] = {}   # name → UUID

    def on_task_request(self, handler: Callable) -> None:
        self._task_handlers.append(handler)

    def on_settings_push(self, handler: Callable[[dict[str, Any]], Any]) -> None:
    ...
    async def get_agent_setting(self, key: str) -> Any | None:
        ...
        async with httpx.AsyncClient(timeout=10.0) as client:
            ...
            return value if value is not None else data.get("default")

    async def _resolve_agent_uuid(self, name_or_id: str) -> str:
        """Return a UUID for the given agent name or pass through if already a UUID."""
        try:
            uuid.UUID(name_or_id)
            return name_or_id   # already a valid UUID
        except ValueError:
            pass

        if name_or_id in self._agent_id_cache:
            return self._agent_id_cache[name_or_id]
        ...
        for agent in agents:
            if agent.get("name") == name_or_id:
                agent_uuid = agent["id"]
                self._agent_id_cache[name_or_id] = agent_uuid
                return agent_uuid

        raise ValueError(f"No agent named {name_or_id!r} found in registry")
    ...
    async def fire_task(
        ...
        """
        Send a task_request fire-and-forget (no waiting for response).
        Use this when the reply will arrive asynchronously via a separate
        capability call (e.g. plan_task replies via send_message).
        """
        if not self._ws:
            raise RuntimeError("Not connected to orchestrator")

        target_uuid = await self._resolve_agent_uuid(target_agent)
    ...
    async def send_task(
        ...
        """
        Send a task_request and await the correlated task_response.
        Use only for capabilities that return a result synchronously.
        """
        if not self._ws:
            raise RuntimeError("Not connected to orchestrator")
        ...
        try:
            ...
            raise TimeoutError(f"Task to {target_agent!r} ({target_uuid}) timed out after {timeout_s}s")
